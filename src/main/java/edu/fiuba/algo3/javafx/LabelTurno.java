...
public class LabelTurno extends Label implements Notificable {
    ...
    public LabelTurno(Jugador jugador) {
        super("\"Turno Jugador: 1");
    ...
    public void actualizar(Jugador jugadorDeTurno) {
        int turno = 1;
        if (jugadorDeTurno != jugador){
            turno = 2;
        }
        if (jugadorDeTurno == null) {
            super.setText("Fin de juego");
            return;
        }
        super.setText("Turno Jugador: " + turno);
    }
}
